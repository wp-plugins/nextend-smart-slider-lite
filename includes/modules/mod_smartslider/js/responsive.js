...
dojo.declare("NextendSmartSliderResponsive", null, {
    ...
    constructor: function(args) {
        ...
        this.cssanimationEnd = animEndEventNames[nModernizr.prefixed('animation')];
        if (args.responsive == 1) {
            this.module_id = parseInt(dojo.attr(args.node, 'id').match(/[0-9]+/)[0]);
        }

        // Disable CSS transition for older browsers...
        if (args.css3transition == 1 && (!nModernizr.csstransitions || !nModernizr.csstransforms3d)) {
            args.css3transition = 0;
        }

        if (args.css3animation == 1 && (!nModernizr.cssanimations || !nModernizr.csstransforms3d)) {
    ...
    commonInit: function() {
        ...
        if (!nModernizr || !nModernizr.backgroundsize) {
