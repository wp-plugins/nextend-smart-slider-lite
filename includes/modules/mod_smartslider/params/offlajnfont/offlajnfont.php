...
class JElementOfflajnFont extends JOfflajnFakeElementBase {
  ...
  function universalfetchElement($name, $value, &$node) {
    ...
    $typexml->addChild('option', array(
      'value' => '0'
    ))->setData('Alternative fonts');
    foreach($this->_google AS $t) {
      ...
      foreach($this->_googlefonts[$t] AS $f) {
        if (strlen($f) > 0) $familyxml->addChild('option', array(
          'value' => $f
        ))->setData($f);
      }
...
EOD;
    ...
    if ($this->_parent->get('admindebug', 0) == 1) {
      ...
      if ($value != "") if ($value[0] != '{') $value = $settings;
