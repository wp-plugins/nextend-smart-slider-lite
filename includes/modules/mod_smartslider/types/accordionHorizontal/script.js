...
dojo.declare("OfflajnSlider", NextendSmartSliderResponsive, {
    constructor: function(args) {
        ...
        if (this.css3transition) {
            for (var i = 0; i < this.dds.length; i++) {
                this.dds[i].addEventListener(this.transitionEnd, dojo.hitch(this, function(e) {
                    if (e.currentTarget == this.dds[this.opened] && e.propertyName == 'width') this.onEnd();
                }));
            }
        }
    ...
    init: function() {
        ...
        this.dts.forEach(function(el, i) {
            el.i = i;
            this.dds[i].i = i;
            if (dojo.contentBox(this.dds[i]).w > 0) {
                this.opened = i;
            }
            ...
            if (this.dds[i].vertical.length > 1) {
                this.dds[i].vertobj = new vertical({
                    ...
                    css3transition: this.css3transition
                });
            }
        }, this);
    ...
    initCaptions: function() {
        ...
        dojo.forEach(this.rawcaptions, function(caps, i) {
            ...
            dojo.forEach(caps, function(c, j) {
                if (c) {
                    ...
                    this.captions[i][j].css3transition = this.css3transition;
                    this.captions[i][j].parent = this.parent;
        ...
        if (verticalObj) {
            if (verticalObj.nodes[verticalObj.current].caption) verticalObj.nodes[verticalObj.current].caption.slideShowed();
        } else {
            var vertical = this.dds[this.opened].vertical;
            if (vertical[0].caption) vertical[0].caption.slideShowed();
        }
    },

    onResponsiveCSSLoaded: function() {
        dojo.attr(this.dds[this.opened], 'style', '');
        this.ddWidth = parseInt(dojo.style(this.dds[this.opened], 'width'));
        this.dds.forEach(function(el, i) {
            if (el.vertobj) {
                var v = el.vertobj;
    ...
    isCurrentSlideAutoplayeable: function(){
      try{
        var vertical = this.dds[this.opened].vertobj;
        if(vertical){
          return this.checkCanvasForAutoplay(vertical.nodes[vertical.current]);  
        }else{
          return this.checkCanvasForAutoplay(this.dds[this.opened]);  
        }
      }catch(e){
        return true;
      }
    ...
    changeSlide: function(nextSlide) {
        ...
        var w = dojo.contentBox(this.dds[nextSlide]).w;
        if (w == 0) {
            if (!this.stopAnim(this.anim)) return;
            dojo.forEach(dojo.query('embed', this.dds[this.opened]), function(v) {
                try {
                    v.stopVideo();
                } catch (e) {};
            });
            ...
            if (this.css3transition) {
                dojo.addClass(this.node, 'nextend-animation');
                dojo.addClass(this.node, 'new-activeslide' + nextSlide);
                dojo.removeClass(this.node, 'new-activeslide' + this.opened);
                this.anim = {
                    status: function() {
                        return "playing";
                    }
    ...
    onEnd: function() {
        if (this.css3transition) {
            dojo.removeClass(this.node, 'nextend-animation');
            this.anim = {
                status: function() {
                    return "stopped";
                }
        ...
        if (verticalObj) {
            if (verticalObj.nodes[verticalObj.current].caption) verticalObj.nodes[verticalObj.current].caption.reset(false);
        } else {
            var vertical = this.dds[this.previous].vertical;
            if (vertical[0].caption) vertical[0].caption.reset(false);
        }
...
dojo.declare("vertical", null, {
    ...
    init: function() {
        ...
        if ((this.showdots == 2) || (this.showdots == 3)) {
            this.aup = dojo.create("div", {
                'class': 'arrowup',
                'innerHTML': '<div class="topline"></div>'
            }, dots);
            dojo.connect(this.aup, "onclick", this, "up");
            this.aupobj = new subslidearrow({
                node: this.aup
            });
            ...
            if (this.showdots == 3) {
                this.numbers = dojo.create("div", {
                    'class': 'numbers',
                    'innerHTML': '<div class="topline"></div><div class="nums">1/' + this.nodes.length + '</div><div class="bottomnline"></div>'
                }, dots);
            ...
            this.adown = dojo.create("div", {
                'class': 'arrowdown',
                'innerHTML': '<div class="bottomline"></div>'
            }, dots);
            dojo.connect(this.adown, "onclick", this, "down");
            this.adownobj = new subslidearrow({
                node: this.adown
            });
        }
        ...
        if (this.css3transition) {
            this.node.addEventListener(this.transitionEnd, dojo.hitch(this, function(e) {
                if (!this.suppressNextCSSEnd) {
                    this.onAnimEnd(this.tmpNextSlide);
                }
                this.anim = {
                    status: function() {
                        return "stopped";
    ...
    onAnimEnd: function(nextSlide) {
        if (this.nodes[this.current].caption) this.nodes[this.current].caption.reset(false);
        if (this.nodes[nextSlide].caption) this.nodes[nextSlide].caption.slideShowed();
