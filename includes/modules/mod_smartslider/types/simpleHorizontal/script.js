...
dojo.declare("OfflajnSliderSimpleHorizontal", NextendSmartSliderResponsive, {
    constructor: function(args) {
        ...
        if (this.css3transition) {
            if (this.transition == 1) {
                this.ul.addEventListener(this.transitionEnd, dojo.hitch(this, 'onEnd'));
            } else if (this.transition == 2) {
                for (var i = 0; i < this.lis.length; i++) {
                    this.lis[i].addEventListener(this.transitionEnd, dojo.hitch(this, function(e) {
                        if (e.currentTarget == this.lis[this.opened] && e.propertyName == 'opacity') this.onEnd();
                    }));
                }
            }
        }
    ...
    init: function() {
        ...
        if (this.transition == 2 && !this.css3transition) {
        ...
        this.width = Math.round(dojo.position(this.lis[0]).w);

        this.previousbtn = dojo.query('.left', this.node);
        if (this.previousbtn[0]) dojo.connect(this.previousbtn[0], 'onclick', dojo.hitch(this, 'onBtn', -1));

        this.next = dojo.query('.right', this.node);
        if (this.next[0]) dojo.connect(this.next[0], 'onclick', dojo.hitch(this, 'onBtn', 1));
    ...
    initCaptions: function() {
        ...
        dojo.forEach(this.rawcaptions, function(c, i) {
            if (c) {
                ...
                this.captions[i].parent = this.parent;
        ...
        if (this.lis[this.opened].caption) this.lis[this.opened].caption.slideShowed();
    },

    onResponsiveCSSLoaded: function() {
        this.width = parseInt(dojo.position(this.lis[this.opened]).w);
        if (this.transition == 1 && !this.css3transition) {
            dojo.style(this.ul, 'marginLeft', -this.width * this.opened + 'px');
        }
    },
    ...
    isCurrentSlideAutoplayeable: function() {
        try {
            return this.checkCanvasForAutoplay(this.lis[this.opened]);
        } catch (e) {
            return true;
        }
    },
    ...
    changeSlide: function(nextSlide) {
        if (!this.stopAnim(this.anim) || nextSlide >= this.n || nextSlide < 0 || nextSlide == this.opened) return;
        dojo.removeClass(this.lis[this.opened], 'selected');

        dojo.forEach(dojo.query('embed', this.lis[this.opened]), function(v) {
            try {
                v.stopVideo();
            } catch (e) {};
        });
    ...
    onEnd: function() {
        if (typeof(this.previous) == "undefined") return;
        if (this.transition == 2 && !this.css3transition) {
            dojo.style(this.lis[this.previous], 'display', 'none');
        }
        if (this.lis[this.opened].caption) this.lis[this.opened].caption.slideShowed();
        if (this.lis[this.previous].caption) this.lis[this.previous].caption.reset(false);
        this.anim = null;
    },
