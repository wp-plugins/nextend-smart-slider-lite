...
class JElementSmartInsert extends JElement {
  ...
  var	$_name = 'smartinsert';
	...
	function fetchElement($name, $value, &$node, $control_name) {
        ...
        DojoLoader::addScriptFile('/administrator/components/com_smartslider/oldparams/smartinsert.js');
        $h = '';
        $html = '';
    ...
    DojoLoader::addScript('new SmartInsertConfigurator({ defaultinsert: dojo.byId("defaultinsert"), h: '.json_encode($h).', sqltables: ' .json_encode($sqlTables).', url: '.json_encode(JURI::base()).'});'); 
    return $html;
	}
}