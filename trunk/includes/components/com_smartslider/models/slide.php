...
class SliderModelSlide extends JModelLegacy{
  ...
  var $_table = '#__offlajn_slide';
    ...
    $prev = $row->ordering;
		...
		$row->content = $data['param']['contenthtml'];
		$row->caption = $data['param']['captionhtml'];
    ...
    if($prev > $cur){
      ...
      . 'WHERE slider = "'.$row->slider.'" AND id != "'.$row->id.'" AND ordering >= "'.($cur).'" AND ordering < "'.($prev).'"';
    }elseif($cur > $prev){
      $query = 'UPDATE #__offlajn_slide '
  		. 'SET ordering = ordering - 1 '
      . 'WHERE slider = "'.$row->slider.'" AND id != "'.$row->id.'" AND ordering <= "'.($cur).'" AND ordering > "'.($prev).'"';
